...
// is resolved once when the overlay's tile URL is built, so it refreshes only on the next app load.
// GIBS "best" serves historical imagery, so in a long-running session the baked
