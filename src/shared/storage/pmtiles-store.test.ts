...
describe('createPmtilesStore', () => {
  it('puts, gets, lists, sums, and deletes with the in-memory fallback (no indexedDB)', async () => {
    const store = createPmtilesStore(undefined);

    await store.put('region-a', blob(10));
    await store.put('region-b', blob(25));

    const got = await store.get('region-a');
    expect(got?.size).toBe(10);

    const list = await store.list();
    expect(list).toEqual([
      { id: 'region-a', byteSize: 10 },
      { id: 'region-b', byteSize: 25 },
    ]);
    expect(await store.totalBytes()).toBe(35);

    await store.delete('region-a');
    expect(await store.get('region-a')).toBeUndefined();
    expect(await store.list()).toEqual([{ id: 'region-b', byteSize: 25 }]);
    expect(await store.totalBytes()).toBe(25);
  });
  ...
  it('degrades to an in-memory map when indexedDB fails to open, never throwing', async () => {
    ...
    expect(await store.totalBytes()).toBe(12);
  });
});
