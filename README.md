...
- Weather: wind, pressure, wave, and precipitation overlays from browser-only Open-Meteo data. Wind
  draws as speed-colored arrows, mean-sea-level pressure as labeled isobar contours (marching squares),
  significant wave height as a smooth color field with direction arrows, and precipitation as a smooth
  rain-rate color field, with a Forecast time slider in the status strip to scrub the coming days and a
  tap-to-read value (wind, pressure, sea state, and rain). Off by default, themed for day, dusk, and
  night-red, and cached for offline. A cloud layer, real-time RainViewer radar, and animated wind
  particles follow.
